...
export const addReview = async (req, res) => {
    var _a;
    try {
        const { productId, userId, reviewText, reviewTitle, starRating } = req.body, token = (_a = req.headers.authorization) === null || _a === void 0 ? void 0 : _a.split(' ')[1];
        ...
        const currentDate = new Date(), newReview = await ReviewModel.create({
            userId,
            productId,
            starRating,
            reviewTitle,
...
export const getReviewsOfProduct = async (req, res) => {
    try {
        const productId = req.params.productId, reviews = await ReviewModel.find({ productId: productId }), product = await ProductModel.findOne({ id: productId });
        if (!product)
            return res.status(404).json({ error: 'Product not found' });
